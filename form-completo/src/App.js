...
function App() {
  const [form, setForm] = useState({ nome: '', email: '', whats: '', data: '', turno: '', vaga: '' });
  const [db, setDb] = useState({ ocupados: [], totalGeral: 0 });
  const [showModal, setShowModal] = useState(false);
  const [msg, setMsg] = useState('');

  const fetchStatus = () => {
    axios.get('http://localhost:3001/status').then(res => setDb(res.data));
  };
  ...
  const enviar = async (e) => {
    e.preventDefault();
    if (!form.vaga) return alert("Por favor, selecione uma vaga disponível.");
    try {
      await axios.post('http://localhost:3001/inscrever', form);
      setShowModal(true);
      setForm({ nome: '', email: '', whats: '', data: '', turno: '', vaga: '' });
      fetchStatus();
    } catch (err) { setMsg(err.response?.data?.message || 'Erro no servidor'); }
  };
    ...
    <div style={containerStyle}>
      <div style={cardStyle}>
        <header style={{ textAlign: 'center', marginBottom: '30px' }}>
          ...
          <p style={{ color: '#64748b' }}>Vagas disponíveis na semana: <strong>{60 - db.totalGeral}</strong></p>
        </header>
        ...
        <form onSubmit={enviar}>
          ...
          <h3 style={{ color: '#334155', marginBottom: '20px' }}>Selecione o dia e uma das 6 vagas do turno:</h3>
            ...
            {AGENDA.map(item => (
              <div key={item.data} style={dayBoxStyle}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  {TURNOS.map(turno => (
                    <div key={turno}>
                      <div style={turnoTitleStyle}>{turno} (0{turno === 'Manhã' ? '9h-11h' : '15h-17h'})</div>
                      <div style={vagasGridStyle}>
                        {VAGAS_LISTA.map(v => {
                          ...
                          return (
                            <button
                              key={v} type="button" disabled={ocupado}
      ...
      {showModal && (
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <p>Sua vaga foi reservada. Nos vemos em breve!</p>
            <button onClick={() => {setShowModal(false); setMsg('')}} style={btnModalStyle}>Fechar</button>
          </div>
        </div>
      )}
    </div>
...
const inputStyle = { padding: '14px', border: '1px solid #cbd5e1', borderRadius: '10px', outline: 'none', fontSize: '15px' };
...
const turnoTitleStyle = { fontSize: '12px', color: '#64748b', marginBottom: '10px', fontWeight: '600', textTransform: 'uppercase' };
const vagasGridStyle = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' };
const vagaButtonStyle = { padding: '10px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', transition: 'all 0.2s' };
const submitButtonStyle = { width: '100%', marginTop: '40px', padding: '18px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(37, 99, 235, 0.2)' };
...
const btnModalStyle = { marginTop: '20px', padding: '12px 40px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' };
